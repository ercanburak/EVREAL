...
# sys.path.append('..')
from ...utils.sample_utils import bilinear_sampler, coords_grid
...
class HalfCorrBlock:
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            dy = torch.linspace(-r, r, 2*r+1)
            delta = torch.stack(torch.meshgrid(dy, dx, indexing='ij'),
                                axis=-1).half().to(coords.device)

            centroid_lvl = coords.reshape(batch*h1*w1, 1, 1, 2) / 2**i
            delta_lvl = delta.view(1, 2*r+1, 2*r+1, 2)
            coords_lvl = centroid_lvl.half() + delta_lvl.half()
    ...
    def corr(fmap1, fmap2):
        batch, dim, ht, wd = fmap1.shape
        fmap1 = fmap1.view(batch, dim, ht*wd).half()
        fmap2 = fmap2.view(batch, dim, ht*wd).half()
